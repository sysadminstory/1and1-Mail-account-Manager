...
class EmailAccountManager (object):
    ...
    listEmailURL = 'https://clients.1and1.fr/CenterCommunication?__render_href=txt/pages/CenterCommunication.xml&__render_part=table-component-body&__render_module=frontend-common&page.size={size}&page.page={page}'
    pageCountURL = 'https://clients.1and1.fr/CenterCommunication?__render_href=txt/pages/CenterCommunication.xml&__render_part=email-overview-pagination-content&__render_module=frontend-common&page.size={size}&page.page={page}&__reuse=1488715767183.__renderinclude__'
    deleteURL = 'https://clients.1and1.fr/CenterCommunication?__render_href=txt/pages/CenterCommunication.xml&__render_part=table-component-body&__render_module=frontend-common&__sendingdata=1&__forcestop=true&__CMD%5B%5D%3ASUBWRP=delete&delete.id={id}'
    accountDetailsURL = 'https://clients.1and1.fr/Email_Summary?id={id}'
    
    userAgent = 'Mozilla/5.0 (Windows NT 10.0; WOW64; rv:51.0) Gecko/20100101 Firefox/51.0' # User agent used during HTTP request
    ...
    def __init__(self, username, password):
        ...
        self.opener = urllib.request.build_opener(urllib.request.HTTPSHandler(context=ctx), *handlers)
    ...
    def authenticate(self):
        ...
        data = data.encode()
        request = urllib.request.Request(EmailAccountManager.loginURL, data, self.headers)
        response = self.opener.open(request)
    ...
    def createAccount(self, data):
        ...
        response = self.doRequest(EmailAccountManager.createEmailURL, None, newAccountData)
        url = response['url']
        responsebody = response['body']
        
        # Check if the email account has been created
        if emailaccount in responsebody:
            print('Account {emailaccount} created successfully'.format(**strData))
        else:
    ...
    def formatURL(self, URL, data):
        """Transform placeholder in predefined URL using the data content and return it as a string"""
        return URL.format(**data)

    def doRequest(self, URL, URLData, postData):
        """Do a request to the control center and return the response URL, content, and a reference to the whole response"""
        if postData != None :
        ...
        response = self.opener.open(request)
        return {'url': response.geturl(), 'body': response.read().decode(), 'self': response}

    def getPageCount(self, URLData):
        """Count the number of pages of the account list"""
        response = self.doRequest(EmailAccountManager.pageCountURL, URLData, None)
        url = response['url']
        responsebody = response['body']
        # If the cast in int fails, then there is only one page
        try:
            tree = html.fromstring(responsebody)
            pageCount = tree.xpath('//ul[@class="content-pagination pagination-a1"]/@data-total-pages')[0]
    ...
    def getAccountList(self):
        ...
        # We only ask the Control Panel if we did not already get the account list
        if self.accountCached == False:
            ...
            # Check the number of page to check, and add every page content to the list
            pageCount = self.getPageCount(URLData)
            for page in range(1, pageCount + 1):
                URLData = {'size': size, 'page': page}
                response = self.doRequest(EmailAccountManager.listEmailURL, URLData, None)
                url = response['url']
                responsebody = response['body']

                # Get the mails name and associated id using xpath
                tree = html.fromstring(responsebody)
                mails = tree.xpath('//a[@class="email-address headline-c1"]/text()')
                ids = tree.xpath('//a[@class="email-address headline-c1"]/@href')
    ...
    def getAccountDetails(self, ID):
        """Get the account Details (First Name, Last Name, Full Name, Account Type, Password)
            Note: Password is not diplayed, therefore only the stars are filled in
            """

        URLData = {'id': ID}
        response = self.doRequest(EmailAccountManager.accountDetailsURL, URLData, None)
        url = response['url']
        responsebody = response['body']
        ...
        email = self.arrayToString(tree.xpath('//*[@id="mamba-group-adresssummary"]/div[2]/table/tbody/tr/td[2]/text()'))
        domainname = email.split('@')[1]
        emailusername = email.split('@')[0]
        accounttype = self.arrayToString(tree.xpath('/html/body/div[1]/div[3]/div/div/form/div/div[2]/div[2]/div[2]/table[1]/tbody/table/tr[1]/td[1]/span/text()'))
        # Get all the Names if this is not a redirect
        if accounttype != 'Redirection':
            emailfirstname = self.arrayToString(tree.xpath('/html/body/div[1]/div[3]/div/div/form/div/div[2]/div[2]/div[2]/table/tbody/table/tr[2]/td/text()'))
            emaillastname = self.arrayToString(tree.xpath('/html/body/div[1]/div[3]/div/div/form/div/div[2]/div[2]/div[2]/table/tbody/table/tr[3]/td/text()'))
            emaildisplayname = self.arrayToString(tree.xpath('/html/body/div[1]/div[3]/div/div/form/div/div[2]/div[2]/div[2]/table/tbody/table/tr[4]/td/text()'))
            emailpassword = self.arrayToString(tree.xpath('/html/body/div[1]/div[3]/div/div/form/div/div[2]/div[2]/div[2]/table/tbody/table/tr[6]/td[1]/span/text()'))
        ...
        data = {'domainname': domainname, 'emailusername': emailusername, 'accounttype': accounttype, 'emailfirstname': emailfirstname, 'emaillastname': emaillastname, 'emaildisplayname': emaildisplayname, 'emailpassword': emailpassword}
        return data
    ...
    def deleteAccountID(self, ID):
        ...
        URLData = {'id': ID}
        response = self.doRequest(EmailAccountManager.deleteURL, URLData, None)
        url = response['url']
        responsebody = response['body']
